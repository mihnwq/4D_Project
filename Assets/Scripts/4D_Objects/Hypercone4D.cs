...
public class Hypercone4D : _4D_Object
{
    ...
    void Start()
    {
        ...
        objectProprieties.Add(new Array2_List("wSlices", wSlices.ToString()));
        objectProprieties.Add(new Array2_List("latSamples", latSamples.ToString()));
        objectProprieties.Add(new Array2_List("lonSamples", lonSamples.ToString()));
        BuildSamples(wSlices,latSamples,lonSamples);
    }
    ...
    /// </summary>
    void BuildSamples(int wSlices, int latSamples, int lonSamples)
    {
    ...
    void DrawHypercone()
    {
        ...
        objectProprieties[0].GetValueInto(ref wSlices);
        objectProprieties[1].GetValueInto(ref latSamples);
        objectProprieties[2].GetValueInto(ref lonSamples);

        wSlices = Mathf.Max(2, wSlices);
        latSamples = Mathf.Max(2, latSamples);
        lonSamples = Mathf.Max(3, lonSamples);

        if (wSlices != this.wSlices || latSamples != this.latSamples || lonSamples != this.lonSamples)
            BuildSamples(wSlices,latSamples,lonSamples);
